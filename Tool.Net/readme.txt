...
foreach (var item in jsonVar)
{
  ...
  ------------2024/12/28------------
  5.4.5 - 正式版 - 优化和性能提升
 1. 优化 NetFrame 接口事件模型，初始并发时的性能提升（不会因创建大量线程导致的任务无法即时消费导致的超时）。
 2. P2p 方面结构优化，成功率进一步提升。
 3. 新增 JsonHelper.IsTryJsonVar 静态属性，使得 JsonVar 更具实用性
 4. JsonVar.IsEnumerator 用于表示是否支持 foreach 语法糖
 5. 优化WebApi参数生成时遵循异步原则
 6. Sockets 模块之前在某种特殊状态下，不会触发断开事件，但实际已经断开并回收了资源，修复了该类问题。
 7. 其他优化

------------2025/01/04------------
5.4.6 - 正式版 - 优化和补偿功能
1. 考虑到udp 通道本就不易成功，进一步优化，提升成功率。
2. 优化功能，因现实环境中，存在服务器地址变更可能性，但目前 连接模块的重连没有 可以控制的事件，无法把每次的重连 都验证原先的地址还是否有效，故增加重连委托，用于更新连接可能存在变更的服务器。
3. 修复 事件枚举设置未生效问题。client.OnInterceptor(EnClient.Receive, true); client.OnIsQueue(EnClient.Fail | EnClient.Connect, true);
4. 其他优化

------------2025/01/13------------
5.4.7 - 正式版 - 优化和补偿功能
1. 优化 JsonVar 对象，提供更多的操作方法，更加方便的使用。
示例：
var  abc = "[1,2,3]".JsonVar();
int[] ints = [.. abc];

abc.GetOrDefault("1", 3); //取不到时返回默认值
2. 优化 JsonEnumerator 对象，提供更多的操作方法，更加方便的使用。
让其 和 JsonVar 一样的使用方式

------------2025/03/03------------
5.5.0 - 正式版
1. 修复在除win操作系统外的文件路径异常问题。
2. QUIC在.net9中完全可用。
