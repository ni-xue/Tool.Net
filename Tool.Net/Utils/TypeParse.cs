...
namespace Tool.Utils
{
    ...
    public sealed partial class TypeParse
    {
        /// <summary>
        /// 无参构造
        /// </summary>
        ...
        public static float StrToFloat(object expression, float defValue)
        {
            ...
            if (expression != null && IsFloatRegex().IsMatch(expression.ToString()))
            {
        ...
        public static int StrToInt(object expression, int defValue)
        {
            ...
            if (text.Length <= 0 || text.Length > 11 || !IsIntRegex().IsMatch(text))
            {
                return defValue;
            }
        ...
        [GeneratedRegex("^([-]|[0-9])[0-9]*(\\.\\w*)?$")]
        private static partial Regex IsFloatRegex();

        [GeneratedRegex("^[-]?[0-9]*$")]
        private static partial Regex IsIntRegex();
#else
        private static Regex IsFloatRegex() => new("^([-]|[0-9])[0-9]*(\\.\\w*)?$");

        private static Regex IsIntRegex() => new("^[-]?[0-9]*$");
#endif
    }
}
