    ...
    public sealed class Validate
    {
        ...
        public static CardDetail GetCardDetail(string cardNo)
        {
            ...
            try
            {
                ...
                {
                    cardDetail = new CardDetail(str["validated"].ToVar<bool>(), cardNo, str["bank"].ToString(), Getownparentbanks(str["bank"].ToString()), str["cardType"].ToString(), "验证成功！");
                }
                else
                {
        ...
        {
            const string JSON = "{\"SRCB\": \"深圳农村商业银行\",\"BGB\": \"广西北部湾银行\",\"SHRCB\": \"上海农村商业银行\",\"BJBANK\": \"北京银行\",\"WHCCB\": \"威海市商业银行\"," +
        ...
        public class CardDetail
        {
            ...
            private Stream GetBankImage()
            {
                // 根据银行缩写,获得银行图片 bank:银行编号,返回支付宝图片对象
                return HttpHelpers.Get($"https://apimg.alipay.com/combo.png?d=cashier&t={Bank}");
            }

            private Stream BankStream { get; set; }

            /// <summary>
            /// 银行图片字节流
            /// </summary>
            public Stream BankImageStream
            {
                get
                {
                    if (BankStream == null)
